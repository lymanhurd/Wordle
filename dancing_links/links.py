...
def make_objects(matrix, row_names=None):
...
def search(header, rows=None, callback=print_rows, level=0):
    ...
    if rows is None:
        rows = []
    logging.info('Searching level %d len rows = %d', level, len(rows))
...
def solve(matrix, callback=print_rows, row_names=None):
    head = make_objects(matrix, row_names=row_names)
